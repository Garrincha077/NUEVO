...
async function loadArchivedMonthlyPrice(root, asset) {
  const file = path.join(root, `research/global-money-v2/transfer/latest/raw/${asset}-yahoo-monthly.json`);
...
function buildMarketRows(prices, cutoffMonth) {
  const common = [...Object.keys(prices[ASSETS[0]])].filter(month => ASSETS.every(a => prices[a][month] != null)).sort();
  const rows = [];
  for (const month of common) {
    ...
    if (!valid) continue;
    const positive = ASSETS.filter(a => perAsset[a]).length;
    rows.push({
      month,
      positive,
      total: 4,
...
export async function buildContextHistory(root, report) {
  ...
  return {
    ...
    market_confirmation: {
      ...
      source_files: ASSETS.map(a => `research/global-money-v2/transfer/latest/raw/${a}-yahoo-monthly.json`),
