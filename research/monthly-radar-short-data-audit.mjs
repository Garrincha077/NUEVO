...
async function fredMonthlyLast(id,start='2003-01-01'){const r=await fetch(`${FRED}?id=${id}&cosd=${start}`,{headers:{'User-Agent':'GMLI-short-audit/1.0'}});if(!r.ok)throw new Error(`FRED ${id} ${r.status}`);const rows=csvRows(await r.text()),by=new Map();for(const x of rows){const date=String(x.observation_date||x.DATE||Object.values(x)[0]),m=date.slice(0,7),v=Number(x[id]);if(m&&Number.isFinite(v)&&m<=cutoff)by.set(m,v)}return by}
const ids=['DFII10','DTWEXBGS','BAMLH0A0HYM2','NFCI','CPIAUCSL'];
const maps={};
...
const common=[...maps.DFII10.keys()].filter(m=>maps.DTWEXBGS.has(m)&&maps.BAMLH0A0HYM2.has(m)&&maps.NFCI.has(m)).sort();
