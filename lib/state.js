export const FROZEN_STATE = {
  as_of: '2026-08-24',
  methodology: {
    status: 'FROZEN_SPEC',
    meaning: 'Weights, lags, horizons, thresholds, train/validation split, FX-neutral methodology and FDR rules are frozen. Data vintages may advance only through an explicit promotion gate.',
    core_auto_update: false
  },
  money: {
    status: 'CORE',
    version: 'GMLI_GLOBAL_MONEY_V2_PBOC_OFFICIAL',
    methodology_status: 'FROZEN_SPEC',
    data_vintage_status: 'VALIDATED_CORE_CURRENT',
    observation_date: '2026-06-30',
    available_date: '2026-07-31',
    coverage: '7/7',
    weighted_coverage: 1,
    usd_yoy_pct: 7.956975,
    fx_neutral_yoy_pct: 5.946277,
    fx_effect_pp: 2.010698,
    usd_z: 0.306729,
    usd_score: 55.1121,
    fxn_z: -0.330721,
    fxn_score: 44.488,
    freshness: 'FRESH',
    source_contract: 'GMLI_GLOBAL_MONEY_V2_PBOC_OFFICIAL',
    note: 'Active Money Core V2 uses the documented seven-region construction with the new audited official-source China V2 path. Historical Core/v1.8b audit facts are preserved below.',
    promotion_candidate: null,
    promotion_gate: {
      status: 'PASS_PROMOTED_TO_CORE',
      core_replacement_allowed: true,
      methodology_changed: false,
      promotion_date: '2026-08-24',
      candidate_version: 'GMLI_GLOBAL_MONEY_V2_PBOC_OFFICIAL',
      promotion_report: 'research/global-money-v2/GMLI_GLOBAL_MONEY_V2_PROMOTION_REPORT.md',
      source_gate: {
        status: 'PASS_CONTINUOUS_OFFICIAL_V2_SOURCE_WITH_COMPARABLE_BASE_SEED',
        china_precision_history: '2015-01 through current official PBoC Money Supply HTML history',
        comparable_base_2014: 'ACCOUNTING_SEED_ONLY',
        observed_stock_2014: false,
        legacy_exact_rerun: false
      },
      may_2026_bridge: {
        status: 'PASS',
        legacy_usd_yoy_pct: 9.3258,
        v2_usd_yoy_pct: 9.341915,
        legacy_fxn_yoy_pct: 6.1275,
        v2_fxn_yoy_pct: 6.153468
      },
      fixed_transmission_transfer: {
        status: 'PASS_FIXED_TRANSMISSION_DIRECTION_TRANSFER',
        passed_relations: 6,
        total_relations: 6,
        train_signal_months: '2015-01..2022-12',
        oos_signal_months: '2023-01+',
        publication_lag_months: 1,
        parameter_search: false,
        lag_search: false,
        horizon_search: false,
        asset_search: false,
        fdr_claim: false
      },
      historical_v18b_gate_status: 'BLOCKED_MISSING_FROZEN_INPUT_BYTES',
      note: 'V2 promotion is an explicit new version decision. It does not convert the historical v1.8b blocked exact-rerun into a pass.'
    },
    historical_reference: {
      status: 'CORE_HISTORICAL_REFERENCE',
      version: 'GMLI_LEGACY_VALIDATED_2026_02',
      available_date: '2026-02-28',
      usd_z: 0.9421742066088,
      usd_score: 65.70290344348,
      fxn_z: -0.4191532308133366,
      fxn_score: 43.01411281977772,
      former_data_vintage_status: 'VALIDATED_CORE_STALE',
      note: 'Last formally validated Core before the explicit Money V2 promotion. Preserved exactly for audit/history.'
    },
    historical_v18b_candidate: {
      status: 'HISTORICAL_PROMOTION_CANDIDATE_BLOCKED',
      evidence_tier: 'RESEARCH',
      version: 'v1.8b-production-money-migration',
      observation_date: '2026-05-31',
      available_date: '2026-06-30',
      coverage: '7/7',
      weighted_coverage: 1,
      usd_yoy_pct: 9.325796746551328,
      fx_neutral_yoy_pct: 6.1275189367599765,
      fx_effect_pp: 3.1982778097913513,
      usd_z: 0.5811506179874022,
      usd_score: 59.68584363312337,
      fxn_z: -0.2809735803930439,
      fxn_score: 45.31710699344927,
      interpretation: 'Historical production-source migration candidate. Its exact promotion audit remains blocked by missing preserved frozen bytes.',
      promotion_gate: {
        status: 'BLOCKED_MISSING_FROZEN_INPUT_BYTES',
        core_replacement_allowed: false,
        methodology_changed: false,
        final_audit_date: '2026-08-17',
        final_audit: {
          decision: 'BLOCKED_MISSING_FROZEN_INPUT_BYTES',
          promote_to_core: false,
          contract_sha256: '0a777dd3f390a4410b409b8b3d650aec125aa59fe5afb749ef9fc34a0f8b0f1f',
          audit_runner_sha256: '31687b1eb40e537042d01a04f34301d31e30caa8f20afbe79224cd2889b1adb9'
        },
        production_source_transfer: {
          key_direction_same: '9/9',
          key_fdr_pass: '6/9',
          dbc_gld_direction_same: '7/7',
          full56_supported_fdr: 6,
          full56_reversed_fdr: 5,
          status: 'PASS_LOCKED_RESULTS'
        },
        australia_source_purity: {
          rba_series: 'DMABMS',
          status: 'PASS',
          migration_months: 138,
          latest_observation: '2026-06-30',
          may_2026_level_aud_bn: 3471.0,
          latest_level_aud_bn: 3499.9,
          june_2026_raw_level_yoy_pct: 7.97828,
          missing_months: 0,
          source_bytes_sha256: 'beb163053d91d6036c032ffbf84d4fac97c418cb13376dfd71ff845871f5b754'
        },
        australia_locked_sensitivity_equivalence: {
          status: 'PASS_WITHIN_LOCKED_15PCT_STRESS_ENVELOPE',
          overlap_months: 135,
          min_rba_over_imf_ratio: 0.8734897778252018,
          max_rba_over_imf_ratio: 1.0096069140211081,
          worst_abs_pct_diff: -12.651022217479824,
          locked_result: '0.85x/1.00x/1.15x all produced supported56=6, reversed56=5, key_dir=9, key_q9=6, dbc_gld_dir=7.'
        },
        exact_frozen_v12_rerun: {
          status: 'NOT_EXECUTED_MISSING_PRESERVED_INPUTS',
          executed: false,
          missing_required_inputs: [
            'v18b-global-money-monthly frozen matrix',
            'v12 exact-ticker adjusted-price frozen mirror',
            'v12 exact-ticker executable runner',
            'v18b full56 migration baseline matrix'
          ],
          reason: 'The exact Aug-15 macro matrix, adjusted-price mirror, original runner and full56 baseline were not preserved as accessible bytes. Current/revised public data cannot be substituted without turning the historical promotion rerun into a new experiment.'
        },
        next_action: 'Historical audit remains closed unless the original frozen bytes are recovered. Do not rebuild the missing vintage from revised data and call it an exact rerun.'
      }
    }
  },
  funding: {
    status: 'RESEARCH_REGIME_OVERLAY',
    available_date: '2026-07-31',
    z: -0.8378753440785458,
    score: 36.035410932024234
  },
  credit: {
    status: 'RESEARCH_TRANSMISSION_OVERLAY',
    available_date: '2026-07-31',
    z: 0.2709184631191401,
    score: 54.51530771865234
  },
  fiscal: {
    status: 'RESEARCH_OVERLAY_ONLY',
    available_date: '2026-07-31',
    z: 0.1523733868591229,
    score: 52.539556447652046,
    mode: 'STRICT_ACTUAL_RELEASE'
  },
  research100: {
    status: 'NOT_VALIDATED_CORE',
    available_date: '2026-02-28',
    usd_score: 54.585023951039496,
    fxn_score: 45.50950770155858,
    freshness: 'STALE_BECAUSE_LEGACY_MONEY'
  },
  transmission: {
    status: 'FROZEN_RESEARCH_RELATIONSHIPS',
    priority: [
      'SPY 12M accel3',
      'QQQ 12M accel3',
      'GLD FX-neutral 12M',
      'DBC USD 6M',
      'DBC USD 12M',
      'DBC FX-neutral 6M'
    ]
  }
};

export function regimeFromScore(score) {
  if (score < 25) return 'STRONG RISK-OFF';
  if (score < 40) return 'RISK-OFF';
  if (score < 60) return 'NEUTRAL';
  if (score < 75) return 'RISK-ON';
  return 'STRONG RISK-ON';
}
