...
function overlayRegime(score) {
  return score < 40 ? 'RESTRICTIVE' : score > 60 ? 'SUPPORTIVE' : 'NEUTRAL';
}
...
export async function buildDecision(opportunity = null) {
  ...
  const fundingRegime = overlayRegime(fundingScore);
  const fiscalScore = FROZEN_STATE.fiscal.score;
  const fiscalRegime = overlayRegime(fiscalScore);
  ...
  return {
    ...
    methodology: {
      ...
      note: 'Money V2 methodology is frozen. Funding V2 and Fiscal V2 are separately promoted frozen OVERLAY contracts. New data vintages may advance only through their promoted fail-closed source contracts; Fiscal V2 has zero automatic weight in the existing 10-point conviction rubric.'
    },
    regime: {
      label: baseLabel,
      score: null,
      ...
      note: 'USD and FX-neutral Money are interpreted separately; no synthetic composite Core score is invented. Fiscal V2 is reported as confirmation context and does not add hidden conviction points.'
    },
    conviction: {
      score: convictionScore,
      max: 10,
      label: `${convictionScore}/10`,
      rubric,
      fiscal_v2_automatic_weight: 0,
      fiscal_v2_note: 'Fiscal V2 is not an additional rubric component; adding automatic Fiscal weight requires a separately frozen decision-engine candidate.'
    },
    money: {
    ...
    fiscal: {
      status: 'OVERLAY',
      role: 'ACTIVE_PROMOTED_FISCAL_V2',
      version: FROZEN_STATE.fiscal.version,
      methodology_status: FROZEN_STATE.fiscal.methodology_status,
      data_vintage_status: FROZEN_STATE.fiscal.data_vintage_status,
      observation_month: FROZEN_STATE.fiscal.observation_month,
      regime: fiscalRegime,
      score: Number(fiscalScore.toFixed(1)),
      deficit_pct_gdp: Number(FROZEN_STATE.fiscal.deficit_pct_gdp.toFixed(2)),
      fiscal_impulse_pp: Number(FROZEN_STATE.fiscal.fiscal_impulse_pp.toFixed(2)),
      available_date: FROZEN_STATE.fiscal.available_date,
      promotion_status: FROZEN_STATE.fiscal.promotion_gate?.status || null,
      empirical_scope: 'Fixed SPY 12M train/OOS directional usefulness gate passed; QQQ/DBC diagnostics are not promotion claims.',
      automatic_global_conviction_weight: 0,
      guardrail: 'Fiscal V2 is confirmation context only in the current engine; it never overrides Money Core and does not silently expand the 10-point conviction rubric.'
    },
    ...
    freshness: {
      ...
      funding_overlay: FROZEN_STATE.funding.available_date,
      fiscal_overlay: FROZEN_STATE.fiscal.available_date,
      warning: moneyFreshness.label === 'STALE'
        ...
        : 'Active Money V2 is current under the promoted official-source contract. Funding V2 is a bounded conviction overlay; Fiscal V2 is a zero-automatic-weight confirmation overlay. Historical legacy blockers do not block their separately versioned V2 contracts.'
    },
    ...
    key_reasons: [
      ...
      `Promoted Fiscal V2 is ${fiscalRegime.toLowerCase()} (${fiscalScore.toFixed(1)}; deficit/GDP ${FROZEN_STATE.fiscal.deficit_pct_gdp.toFixed(2)}%, 12M impulse ${FROZEN_STATE.fiscal.fiscal_impulse_pp.toFixed(2)} pp). It passed the fixed SPY 12M usefulness gate but carries zero automatic conviction weight.`,
    ...
    invalidation_triggers: [
      ...
      'A promoted Money V2, Funding V2 or Fiscal V2 source-refresh contract fails validation or stops advancing; preserve last-good state and reduce confidence according to the affected evidence tier.'
    ]
  };
}
