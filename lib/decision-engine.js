...
export async function buildDecision(opportunity = null) {
  ...
  return {
    schema_version: 'gmli-decision-v4.0',
    engine_version: 'GMLI 2.4.0',
    as_of: new Date().toISOString().slice(0, 10),
    methodology: {
      status: FROZEN_STATE.methodology?.status || 'FROZEN_V2_SPEC',
      note: 'Money V2 methodology is frozen. New data vintages may advance only through the promoted official-source contract and fail-closed guards.'
    },
    regime: {
      label: baseLabel,
      score: null,
    ...
    freshness: {
      ...
      warning: moneyFreshness.label === 'STALE'
        ...
        : 'Active Money V2 is current under the promoted official-source contract. The old v1.8b blocker is historical audit context only.'
    },
    ...
    key_reasons: [
      ...
      `Funding remains ${fundingRegime.toLowerCase()}, limiting a stronger risk-on conclusion.`,
    ...
    invalidation_triggers: [
      ...
      'Funding remains restrictive while current market confirmation turns broadly negative.',
      'The promoted source-refresh contract fails validation or stops advancing, causing active Money freshness to become stale.'
    ]
  };
}
