...
function item(key, label, tier, date, now, extra = {}) {
  ...
  return { key, label, evidence_tier: tier, available_date: date || null, age_days, freshness: freshnessLabel(age_days), ...extra };
}
...
export function buildFreshnessHealth(decision, opportunity, generatedAt = new Date()) {
  ...
  const positioningDates = coreAssets.map(k => opportunity?.assets?.[k]?.entry_inputs?.positioning?.as_of).filter(Boolean);

  const items = [
    item('money_core', 'Active Money Core V2', 'CORE', FROZEN_STATE.money.available_date, now, {
      decision_critical: true,
      version: FROZEN_STATE.money.version,
      refresh_contract: FROZEN_STATE.money.source_contract,
      promotion_status: FROZEN_STATE.money.promotion_gate?.status || 'UNKNOWN'
    }),
    item('money_historical_reference', 'Pre-V2 validated Core reference', 'CORE_HISTORICAL_REFERENCE', FROZEN_STATE.money.historical_reference?.available_date, now, {
      decision_critical: false,
      active: false,
      note: 'Audit/comparison only; no longer used as active Money Core.'
    }),
    item('money_legacy_v18b', 'Historical v1.8b migration candidate', 'RESEARCH_HISTORICAL', FROZEN_STATE.money.historical_v18b_candidate?.available_date, now, {
      decision_critical: false,
      active: false,
      promotion_gate: FROZEN_STATE.money.historical_v18b_candidate?.promotion_gate?.status || 'UNKNOWN'
    }),
    ...
    item('funding', 'Funding overlay', 'OVERLAY', FROZEN_STATE.funding.available_date, now, { decision_critical: true, ...overlayMeta('funding') }),
    ...
    item('market_structure', 'Core asset completed-month structure', 'RESEARCH', minDate(marketDates), now, { decision_critical: true, assets: coreAssets }),
    item('cftc_positioning', 'Core asset CFTC positioning', 'RESEARCH', minDate(positioningDates), now, { decision_critical: false, role: 'ENTRY_QUALITY_ONLY' })
  ];
  ...
  const status = staleCritical.length
    ...
    : agingCritical.length ? 'AGING' : 'HEALTHY';
  ...
  return {
    policy: {
      ...
      aging_max_days: 60,
      stale_min_days: 61,
      note: 'Freshness changes conviction. The promoted Money V2 methodology is frozen, while validated data vintages may advance under its official-source refresh contract.'
    },
    status,
    generated_at: now.toISOString(),
    active_money_version: FROZEN_STATE.money.version,
    oldest_decision_critical_input: oldest ? {
      key: oldest.key, label: oldest.label, available_date: oldest.available_date, age_days: oldest.age_days, freshness: oldest.freshness
    } : null,
    counts: {
    ...
    overlay_refreshability: {
      ...
      blocked_or_pending: overlays.filter(x => !x.refreshable).map(x => ({
        key: x.key, status: x.automation_status, last_good_date: x.last_good_date, blocker: x.refresh_blocker
      }))
    },
    items
  };
